...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseServer
      .from("transactions")
      .insert([{ type, amount, date, user_id: null }])
      .select()
      .single();
...
export async function PATCH(req: NextRequest) {
  try {
    const body = await req.json();
    const { id, amount, date } = body;

    // Validation
    if (!id || !amount || !date) {
      return NextResponse.json({ error: "Missing required fields: id, amount, and date are required" }, { status: 400 });
    }
